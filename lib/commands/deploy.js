      ...
      return site.createDeploy(options).then(function(deploy) {
        ...
        return deploy.waitForReady().then(function(deploy) {
          ...
          if (cmd.draft) {
            ...
            var WRITE_PERMALINK_TO_FILE = process.env.WRITE_PERMALINK_TO_FILE;
            if (WRITE_PERMALINK_TO_FILE) {
              var path = require('path');
              var permalinkFilePath = path.join(process.cwd, WRITE_PERMALINK_TO_FILE);
              console.log('Writing url to file:', permalinkFilePath);
              fs.writeFileSync(permalinkFilePath, deploy.deploy_url);
              console.log('DONE!');
            }
